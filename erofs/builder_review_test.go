...
// bumpSlab allocates the way the daemon's AllocateBatch does: a bump pointer
// that advances by exactly the requested block count.
...
func buildNoPanic(m *pb.Manifest, sm SlabManager) (err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	return NewBuilder(BuilderConfig{BlockShift: 12}).BuildFromManifestWithSlab(context.Background(), m, &out, sm)
}

// erofs reads a full chunk at the chunk index's block address, including the
// last chunk of a file whose size is an exact multiple of the chunk size. The
// builder must ask the slab for that many blocks, or the next chunk is
// allocated at the same address.
func TestReviewBuildExactMultipleChunkGetsBlocks(t *testing.T) {
	cs := shift.DefaultChunkShift
	...
	sm := &bumpSlab{}
	require.NoError(t, buildNoPanic(m, sm))
...
// Symlink targets up to 4095 bytes are valid in a nar (go-nix pathLenMax).
// The builder has to lay them out, or reject them, without corrupting the
// inode layout.
func TestReviewBuildLongSymlink(t *testing.T) {
	for _, n := range []int{4064, 4065, 4095} {
		m := &pb.Manifest{Entries: []*pb.Entry{
			{Path: "/", Type: pb.EntryType_DIRECTORY},
			{Path: "/link", Type: pb.EntryType_SYMLINK, InlineData: []byte(strings.Repeat("x", n))},
			{Path: "/z", Type: pb.EntryType_SYMLINK, InlineData: []byte("target")},
		}}
		err := buildNoPanic(m, &bumpSlab{})
		require.NotContains(t, fmt.Sprint(err), "panic:", "symlink target of %d bytes", n)
	}
}
