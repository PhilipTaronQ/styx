...
func gcTestStorePath(c byte, name string) string { return testSph(c) + "-" + name }

// Gives s a fake devnode so mounts are accepted. Nothing reads the other end.
func gcTestDevnode(t *testing.T, s *Server) {
	t.Helper()
	dev, other := testDevnode(t)
	t.Cleanup(func() { unix.Close(dev); unix.Close(other) })
	s.devnode.Store(int32(dev))
}
...
func TestGcAfterFailedMount(t *testing.T) {
	...
	img := gcTestGetImage(t, s, testSph('0'))
...
func TestRejectedConcurrentMountKeepsRecord(t *testing.T) {
	...
	img := gcTestGetImage(t, s, testSph('1'))
	release()
	<-firstDone
...
func TestGcErrorStatesKeepsMountInProgress(t *testing.T) {
	...
	require.NotNil(t, gcTestGetImage(t, s, testSph('2')))
...
func TestGcKeepsHeldImage(t *testing.T) {
	s := newTestServer(t, 2, true)
	initTestServer(t, s, "http://localhost:1")
	d := gcTestDigest(3)
	gcTestImage(t, s, '3', "pkg-1.0", pb.MountState_Unmounted, d)

	release := s.holdForGc(testSph('3'))
...
func TestCompactionKeepsDb(t *testing.T) {
	cfg := Config{CachePath: t.TempDir(), ErofsBlockShift: 12, FdStore: testFdStore{}}
	sphStr := testSph('c')
	s := NewServer(cfg)
...
func TestCatalogSkipsStaleBase(t *testing.T) {
	...
	reqSph, _, err := ParseSph(testSph('9'))
	require.NoError(t, err)
...
func TestGcDuringVaporizeKeepsItsSpace(t *testing.T) {
	...
	garbageSph, _, err := ParseSph(testSph('4'))
	require.NoError(t, err)
	...
	vapSph, vapSphStr, err := ParseSph(testSph('5'))
	require.NoError(t, err)
...
func TestGcWaitsForBusyImageUnlocked(t *testing.T) {
	...
	asked := gcTestInUse(t, testSph('k'), func() bool { return true })
	...
	start := time.Now()
	require.NoError(t, s.imageTx(testSph('l'), func(img *pb.DbImage) error {
...
func TestGcKeepsImageBusyAfterWait(t *testing.T) {
	...
	gcTestInUse(t, testSph('n'), func() bool { return asks.Add(1) > 1 })
...
func TestGcWaitsForImageRelease(t *testing.T) {
	...
	var asks atomic.Int32
	gcTestInUse(t, testSph('p'), func() bool { return asks.Add(1) <= 3 })
