...
// workers; a worker serving a kernel read waits in requestChunk on a fetch
// that runs on context.Background (and RetryHttpRequest retries connection
// errors forever), so a stalled chunk store keeps Stop from returning.
